...
************************************************************************ */
qx.Class.define("myapp.components.MenuBar", {
    ...
    construct: function () {
        ...
        var navbarCard = new myapp.components.ui.Card("", "", false);
    ...
    members: {
        ...
        setCompactMode: function (compact) {
            ...
            if (this._isCompactMode === isCompact)
                return;
            this._isCompactMode = isCompact;
            if (this._windowsMenuButton)
                this._windowsMenuButton.setLabel(isCompact ? "Forms" : "Students");
            if (this._windowMenuButton)
                this._windowMenuButton.setLabel(isCompact ? "Layout" : "Window");
            if (this._printingMenuButton)
                this._printingMenuButton.setLabel(isCompact ? "Print" : "Printing");
            if (this._demoMenuButton)
                this._demoMenuButton.setLabel("Demo");
        ...
        _applyBasecoatMenuBarStyles: function (menubar) {
            ...
            menubar.addListenerOnce("appear", () => {
                ...
                if (!element)
                    return;
                myapp.util.Theme.styleDOMElement(element, {
        ...
        _styleMenuBarButton: function (button, options) {
            if (!button || button.__basecoatMenuStyled)
                return;
        ...
        _stylePopupMenuItem: function (menuItem) {
            if (!menuItem || menuItem.__basecoatMenuItemStyled)
                return;
        ...
        _stylePopupMenu: function (menu) {
            if (!menu || menu.__basecoatPopupStyled)
                return;
            ...
            const applyPopupStyles = () => {
                ...
                const isSeparatorChild = (entry) => {
                    ...
                    return entry instanceof myapp.components.ui.MenuSeparator
                        || entryClassName.indexOf("qx.ui.menu.Separator") !== -1;
                };
                children.forEach((child) => {
                    const className = child.classname || "";
                    const isCustomSeparator = child instanceof myapp.components.ui.MenuSeparator;
        ...
        _ensureFeedbackUI: function () {
            if (this._feedbackToast && this._feedbackDialog)
                return;
            ...
            if (!this._feedbackToast) {
                this._feedbackToast = new myapp.components.ui.Toast();
                root.add(this._feedbackToast, { edge: 0 });
            }
            if (!this._feedbackDialog) {
                this._feedbackDialog = new myapp.components.ui.Dialog("Notice", "");
         ...
         * @param {myapp.components.WindowManager} windowManager
         */
        ...
        _loadScriptFromNodeModules: function (modulePath, callback, errorCallback) {
            ...
            script.onerror = () => {
                ...
                if (errorCallback) {
                    ...
                    this._notifyError("Failed to load script from node_modules", modulePath + "\nPath: " + fullPath + "\nNote: Dev server must serve node_modules.");
                }
            };
            // Append to document head
            document.head.appendChild(script);
        ...
        _loadScript: function (scriptPath, callback, errorCallback) {
            ...
            try {
                ...
                fullPath = resourceManager.toUri("myapp/lib/" + scriptPath);
                ...
                if (pathname.includes("/compiled/")) {
                    ...
                    fullPath = urlObj.origin + pathname.substring(0, compiledIndex) + "/resource/myapp/lib/" + scriptPath;
                }
                else {
                    // For source mode with qx serve, resources are typically at /resource/
                    fullPath = urlObj.origin + "/resource/myapp/lib/" + scriptPath;
                }
            }
