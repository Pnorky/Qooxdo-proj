qx.Class.define("myapp.components.ui.Label", {
  ...
  members: {
    _html: null as any,
    _labelElement: null as HTMLLabelElement | null,
    _initialValue: null as string | null,

    _getLabelElement(): HTMLLabelElement | null {
    ...
    _applyFontStyle(fontStyle: "bold" | "italic" | null): void {
      const label = this._getLabelElement();
      if (!label) return;
      if (fontStyle === "bold") {
    ...
    _applyTextAlign(align: "left" | "center" | "right" | "justify"): void {
      const label = this._getLabelElement();
      if (label) {
