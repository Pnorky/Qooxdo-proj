    ...
    statics: {
        ...
        _getSpriteUri() {
            if (!myapp.util.Lucide.__spriteUri) {
                myapp.util.Lucide.__spriteUri = qx.util.ResourceManager.getInstance().toUri(myapp.util.LucideRegistry.SPRITE_RESOURCE_ID);
            }
        ...
        svgHtml(name, options) {
            ...
            const size = options && options.size != null && !isNaN(Number(options.size))
                ? Number(options.size)
                : 18;
            ...
            const aria = options && options.ariaHidden === false
                ? ""
            ...
            return (`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="${cls}"` +
