...
qx.Class.define("myapp.Application", {
  ...
  members: {
    ...
    __mainView: null as any,
    __contentStack: null as any,
    __menuBar: null as any,
    __sidebar: null as any,
    __bodyShell: null as any,
    __contentRow: null as any,
    __drawerBackdrop: null as any,
    __shellMobileDrawer: false,
    __mobileSidebarVisible: false,
    __drawerBackdropClickBound: null as any,
    __mobileBreakpoint: 900,

    __screens: null as any,
    __personalTab: null as any,
    __contactTab: null as any,
    __academicTab: null as any,
    __studentTableTab: null as any,
    __uiDemoTab: null as any,
    __uiTabToastDemoTab: null as any,
    ...
    _buildMainView(): any {
      ...
      this.__sidebar.addListener("openWindowRequest", (e: any) => {
      ...
      this.__drawerBackdrop = new qx.ui.embed.Html(
        "<div class=\"app-mobile-drawer-backdrop\" aria-hidden=\"true\"></div>"
      );
    ...
    _createTabRouter(): any {
      return {
        openWindow: (key: string) => this._showScreen(key),
        closeAllWindows: () => {},
        cascadeWindows: () => {},
        tileWindows: () => {},
        getWindow: (key: string) => {
          if (key !== "studentTable") return null;
          return {
    ...
    _showScreen(screenKey: string): void {
      if (!this.__screens) return;
    ...
    _syncMobileDrawerVisuals(): void {
      if (!this.__shellMobileDrawer || !this.__sidebar) return;
    ...
    _applyMobileDrawerShell(compact: boolean): void {
      if (!this.__bodyShell || !this.__contentRow || !this.__sidebar || !this.__drawerBackdrop) return;
    ...
    _bindMobileBackdropClick(): void {
      const wrap = this.__drawerBackdrop?.getContentElement?.()?.getDomElement?.();
      if (!wrap || !this.__drawerBackdropClickBound) return;
      const inner = wrap.querySelector(".app-mobile-drawer-backdrop") as HTMLElement | null;
      if (!inner || inner.dataset.appBackdropBound === "1") return;
      inner.addEventListener("click", this.__drawerBackdropClickBound);
      inner.dataset.appBackdropBound = "1";
    },

    _unbindMobileBackdropClick(): void {
      const wrap = this.__drawerBackdrop?.getContentElement?.()?.getDomElement?.();
      if (!wrap || !this.__drawerBackdropClickBound) return;
      const inner = wrap.querySelector(".app-mobile-drawer-backdrop") as HTMLElement | null;
      if (!inner || inner.dataset.appBackdropBound !== "1") return;
